...
- **网络访问：** 用户执行发布时，插件会将当前笔记的 Markdown 和其中引用的本地图片发送到所配置的 API 服务器，以创建或更新分享页面；删除分享也会访问该服务器。
- **激活统计：** 插件加载时会向所配置的 API 服务器发送插件版本、平台、时间戳和一个随机生成的 vault 专属匿名 ID；不会包含笔记内容、笔记路径、vault 名称、用户名或主机名。
...
- **本地数据：** 设置、分享更新令牌和图片缓存记录通过 Obsidian 插件数据 API 保存；匿名激活 ID 通过 Obsidian 的 vault 级本地存储 API 保存。
- 游客分享链接 24 小时后自动过期。
