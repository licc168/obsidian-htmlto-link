...
- **保存时自动更新** — 可选。已分享的笔记在保存后覆盖同一条链接；第一次仍需手动分享
- **30+ 模板** — 备忘录、波普艺术、中国传统、线圈笔记本、赛博朋克、玻璃拟态等
- **主题变体** — 每个模板支持多种配色（如备忘录的明亮/暗黑模式）
- **本地模板预览** — 在当前 Markdown 页面右上角直接切换模板和主题；预览不发布、不上传笔记
- **自适应宽度** — 分享页随屏幕宽度自动适配
...
1. 打开任意 Markdown 笔记。
2. 在页面右上角使用 **模板** 和 **主题** 下拉框，即可在当前页面本地预览；选择 **Obsidian 原样** 可退出预览。
3. 要公开分享时，再通过 `Ctrl/Cmd + P` 搜索 **分享当前笔记**（或点击左侧功能区的分享图标，或在文件列表右键笔记）。
4. 发布弹窗中确认模板/主题，未保存的编辑也会一并发布，分享链接自动复制到剪贴板。

删除分享：`Ctrl/Cmd + P` → **删除当前笔记的分享**。

若希望公开页跟着笔记走：先手动分享一次，再在插件设置里打开 **保存时自动更新分享**。之后保存会在短暂停顿后更新原链接；旧链接过期时不会自动改成新网址。
...
| 保存时自动更新分享 | 默认关闭。已分享的笔记在保存后更新同一条公开链接 |
| 发布时弹出选项框 | 模板和主题选择对话框 |

模板预览不单独占用设置项，默认通过 Markdown 页面右上角控件启用；它与发布流程分开。

## 隐私与数据

- **网络访问：** 模板预览完全在本地完成，不发送笔记正文，也不上传本地图片。用户主动执行发布时，插件才会将当前笔记的 Markdown 和其中引用的本地图片发送到所配置的 API 服务器，以创建或更新分享页面。若开启「保存时自动更新分享」，保存已分享笔记时也会上传该笔记及其中引用的本地图片以更新原页面。删除分享也会访问该服务器。插件不会扫描或上传 Vault 中的其他笔记。
...
- **剪贴板访问：** 插件只写入剪贴板，不会读取。发布成功且开启「成功后复制链接」时，会把公开 URL 写入剪贴板；本地模板预览里点击代码块也可复制该代码。因此不会接触你在 Obsidian 之外复制的内容。
...
│   ├── api.ts         # API 客户端
│   ├── publish.ts     # 分享与删除逻辑
│   ├── auto-update.ts # 保存时自动更新
│   ├── template-preview/ # 本地模板预览
│   └── i18n.ts        # 国际化
├── scripts/
│   └── sync-template-assets.mjs # 同步 html2url 模板 CSS
└── README.md
```
